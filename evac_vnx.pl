...
foreach my $nv (sort keys %lun) {
        ...
        my $vol_create = sprintf "na-adcna volume create -vserverVSERVER -volume %s -aggregate AGGREGATE -size %sGB -foreground true", $nv, $vol_size;
        print "$vol_create\n";
        system($vol_create);
        foreach my $rec ( @{$lun{$nv}} ) {
                ...
                my @storage_volumes = `vplex_array_rediscover_netapp_florence.exp`;
                print "DONE!\n";
                ...
                chomp($sext);
                my $mcmd = "vplex_start_extent_migration.exp $$rec[0] $sext extent_ADCAFF_" . $$rec[0];
                print "$mcmd\n";
                system($mcmd);
        }
}
